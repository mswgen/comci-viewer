            ...
            <div className='grid grid-cols-[auto_1fr_auto]'>
                ...
                <button onClick={() => {
                    if ('clipboard' in navigator) {
                        navigator.clipboard.writeText(`${location.origin}/teacher/${classData.school.code}/${classData.teacher.code + 1}`).then(() => {
                            setIsCopied(true);
                            setLastCopied(Date.now());
                        }).catch(() => {
                            setDialogType('alert');
                            setDialogTitle('클립보드에 복사할 수 없음');
                            setDialogContent('이 브라우저는 클립보드에 복사 기능을 지원하지만 알 수 없는 오류로 인해 현재 복사할 수 없습니다.\n아래 링크를 수동으로 복사해주세요.\n\n' + `${location.origin}/teacher/${classData.school.code}/${classData.teacher.code + 1}`);
                            setShowDialog(true);
                        });
                    } else {
                        setDialogType('alert');
